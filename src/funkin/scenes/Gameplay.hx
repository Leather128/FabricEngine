...
class Gameplay extends FunkinScene {
	...
	private function set_note_input(value:haxe.Constraints.Function):haxe.Constraints.Function {
		if (ui != null)
			ui.note_input = function():Void {
				Reflect.callMethod(null, value, [ui]);
			};
		note_input = value;
		return value;
	}
	...
	override function create():Void {
		...
		scripts.push(bf.script);

		// for tutorial type shit
		if (dad.character == gf.character) {
			scripts.remove(dad.script);
			remove(dad);
			dad.destroy();
			dad = gf;
		}
		...
		// add ui to screen
		ui = new funkin.sprites.ui.GameplayUI();
		ui.scrollFactor.set();
		ui.cameras = [hud_cam];
		default_input = ui.note_input;

		if (note_input == null)
			note_input = default_input;
		else
			note_input = note_input; // this sets ui.note_input to note_input
		add(ui);
		...
		call_scripts('create_post');
		call_scripts('createPost');
	...
	override function update(elapsed:Float):Void {
		...
		if (started_countdown && !song_started && Conductor.song_position >= 0.0)
			play_song();

		// syncing
		if (song_started
			&& (Math.abs(FlxG.sound.music.time - Conductor.song_position_raw) >= 25.0
				|| (vocals.playing && Math.abs(vocals.time - Conductor.song_position_raw) >= 25.0)))
			resync_song();
		...
		call_scripts('update_post', [elapsed]);
		call_scripts('updatePost', [elapsed]);
	}
	...
	public function start_countdown():Void {
		call_scripts('start_countdown');
		call_scripts('startCountdown');
	...
	public function play_song():Void {
		call_scripts('on_play');
		call_scripts('play_song');
		call_scripts('playSong');

		// loading vocals
		if (song.needsVoices != false)
			vocals = new FlxSound().loadEmbedded(Assets.audio('songs/${song.song.toLowerCase()}/Voices'));
		else {
			vocals = new FlxSound();
			vocals.volume = 0.0;
		}
	...
	public function end_song():Void {
		call_scripts('on_end');
		call_scripts('end_song');
		call_scripts('endSong');
	...
	public function resync_song():Void {
		call_scripts('on_resync');
		call_scripts('resync_song');
		call_scripts('resyncSong');
		...
		call_scripts('on_resync_post');
		call_scripts('resync_song_post');
		call_scripts('resyncSongPost');
	}
