...
class MainMenu extends FunkinScene {
    ...
    override function create():Void {
        ...
        if (!FlxG.sound.music.playing) TitleScreen.play_music();
        ...
        magenta.scrollFactor.copyFrom(bg.scrollFactor); magenta.scale.copyFrom(bg.scale);
        ...
        var story_mode:AtlasItem = menu_list.add_item(0, 0, 'menus/main/items', {
            ...
            selected: {name: 'story mode selected', framerate: 24, looped: true
        }}); story_mode.scrollFactor.set();
        story_mode.on_selected.add(function(item:AtlasItem):Void { switch_scene(new StoryMenu()); });

        var freeplay:AtlasItem = menu_list.add_item(0, 0, 'menus/main/items', {
            unselected: {name: 'freeplay idle', framerate: 24, looped: true},
            selected: {name: 'freeplay selected', framerate: 24, looped: true
        }}); freeplay.scrollFactor.set();
        freeplay.on_selected.add(function(item:AtlasItem):Void { switch_scene(new Freeplay()); });

        var options:AtlasItem = menu_list.add_item(0, 0, 'menus/main/items', {
            unselected: {name: 'options idle', framerate: 24, looped: true},
            selected: {name: 'options selected', framerate: 24, looped: true
        }}); options.scrollFactor.set();
        options.on_selected.add(function(item:AtlasItem):Void { switch_scene(new OptionsMenu()); });

        // Position the items
        var initial_y:Float = (FlxG.height - 160.0 * (menu_list.length - 1.0)) / 2.0;

		for (i in 0...menu_list.length) menu_list.members[i].setPosition(FlxG.width / 2.0, initial_y + (160.0 * i));

        // Add some event handlers
        menu_list.on_select.add(function(item:AtlasItem):Void { cam_follow.setPosition(item.getGraphicMidpoint().x, item.getGraphicMidpoint().y); });
        menu_list.on_accept.add(function(item:AtlasItem):Void { flixel.effects.FlxFlicker.flicker(magenta, 1.1, 0.15, false, true); });
    ...
    override function update(elapsed:Float):Void {
        ...
        if (Input.is('nine')) FlxG.switchState(new ScriptedScene('Test'));
    }
    ...
    public function switch_scene(next_scene:FunkinScene):Void {
        ...
        new flixel.util.FlxTimer().start(0.4, function(_):Void { FlxG.switchState(next_scene); });
    }
}
