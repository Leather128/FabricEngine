...
/**
 * `FunkinSubScene` that uses a script to run.
 * @author Leather128
 */
class ScriptedSubScene extends FunkinSubScene {
	/**
	...
	/**
	 * Whether or not to run the default code of a state (used on custom overrides to preexisting states)
	 */
	public var run_default:Bool = true;

	public function new(?scene_path:String = null) {
		super();

		if (scene_path == null)
			scene_path = StringUtils.class_name(this);

		// loads da script
		script = Script.load('scenes/${scene_path}', false);
		// no nulls
		if (script == null) {
			trace('Using dummy script for ${scene_path} as no script was found!', WARNING);
			script = new Script();
		}
	...
	override function update(elapsed:Float):Void {
		...
		script.call('update_post', [elapsed]);
	...
	override public function onFocus():Void {
		script.call('on_focus');
		script.call('onFocus');
		super.onFocus();
	}

	override public function onFocusLost():Void {
		script.call('on_focus_lost');
		script.call('onFocusLost');
		super.onFocusLost();
	}

	override public function closeSubState():Void {
		script.call('close_sub_state');
		script.call('closeSubState');
		super.closeSubState();
	}

	override public function openSubState(sub_state:flixel.FlxSubState):Void {
		script.call('open_sub_state', [sub_state]);
		script.call('openSubState', [sub_state]);
		super.openSubState(sub_state);
	}

	override public function resetSubState():Void {
		script.call('reset_sub_state');
		script.call('resetSubState');
		super.resetSubState();
	}

	override public function switchTo(next_state:flixel.FlxState):Bool {
		script.call('switch_to', [next_state]);
		script.call('switchTo', [next_state]);
		return super.switchTo(next_state);
	}

	override public function onResize(width:Int, height:Int):Void {
		script.call('on_resize', [width, height]);
